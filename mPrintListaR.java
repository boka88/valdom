...
public class mPrintListaR {
	...
	private String oddat,dodat,naziv=" ",nazradnika;
  ...
  public mPrintListaR(Connection _connection,
						String _oddat,
						String _dodat,
						double _koeficient,
						String _nazradnika) {
...
private void writeHeader() 
{
	...
	// pocinju stavke  -------------------------------------------------------------------------
		output.write("      <fo:region-body margin-top=\"50mm\"\n"); 
	...
	//-------------------- prva stavka je visina u koju mora stati zaglavlje   -------------
		output.write("      <fo:region-before extent=\"50mm\"/>\n"); 
...
//------------------------------------------------------------------------------------------
 private void writeDetails() {
        ...
        try {
			...
			output.write("              <fo:block>" + konvUnicode("Lista radnih naloga") + "\n"); 
			output.write("              </fo:block>\n"); 
			output.write("            </fo:table-cell>\n"); 
			// strana -----------------------------------------------------------------------
			output.write("          </fo:table-row>\n"); 
			...
			// vrsta knjizenja -----------------------------------------------------------
			output.write("            <fo:table-cell text-align=\"left\">\n"); 
			output.write("              <fo:block>" + konvUnicode(nazradnika) + "\n"); 
			output.write("              </fo:block>\n"); 
			output.write("            </fo:table-cell>\n"); 
			// crtica  -----------------------------------------------------------------------
			output.write("          </fo:table-row>\n"); 
...
//---------------------------------------------------------------------------------
 private void popuniPodatkeIzBaze() {
	...
	String sql = "SELECT rbr,brsasije,marka,tip,datum,ROUND(cenarada,2),ROUND(cenadelova,2) " +
     ...
     try {
        ...
        while ( rs.next() ) {
			  ...
			  String[] record = new String[7];
				...
				record[2] = konvertujDatumIzPodataka(rs.getString("datum"));
				record[3] = rs.getString("ROUND(cenarada,2)");
				record[4] = rs.getString("ROUND(cenadelova,2)");
				record[5] = rs.getString("marka");
				record[6] = rs.getString("tip");
				zarada = myFormatter.format(rs.getDouble("ROUND(cenarada,2)") * koeficient / 100);
				ukuzarada = ukuzarada + rs.getDouble("ROUND(cenarada,2)") * koeficient / 100;
...
//---------------------------------------------------------------------------------
 private void popuniUkupno() {
		  try {
			...
			output.write("	<fo:table-column column-width=\"7.5cm\"/>\n");
			output.write("	<fo:table-column column-width=\"2cm\"/>\n");
			output.write("	<fo:table-column column-width=\"2cm\"/>\n");
			output.write("	<fo:table-column column-width=\"2cm\"/>\n");
			output.write("    <fo:table-body>\n"); 
