...
public class jPrintPocStanje {
	...
	private String registracija,vrstavozila,jur="",mag="";
	private int radnik=0;

	public jPrintPocStanje(Connection _connection, int _jur,int _mag){
		connection = _connection;
		jur = String.valueOf(_jur);
		mag = String.valueOf(_mag);

		nadjiPreduzece();

		try
		{
			JasperDesign jasperDesign = JasperManager.loadXmlDesign("print/pocetnostanje.jrxml");
			...
			parameters.put("jur", jur);
			parameters.put("mag", mag);
...
//--------------------------------------------------------------------------------------
	private JasperPrint uzmiPodatke(JasperReport _jasperReport,Map _parameters){
		...
		String sql = "SELECT * FROM mprom1 WHERE vrdok=0 AND brun=0 ORDER BY rbr";
		try {
