...
public class mKartice extends JInternalFrame implements InternalFrameListener
			{
	...
	private JList list;
   	int n_fields,kontomal;
//------------------------------------------------------------------------------------------------------------------
    public mKartice() {
		super("", true, true, true, true);
		setTitle("Materijalne kartice ");
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildNazivPanel() {
		...
		mkdisplej1 = new JLabel("                       "); 
		mkdisplej1.setFont(new Font("Arial",Font.BOLD,14));
		mkdisplej1.setBackground(new Color(90,105,108));
		mkdisplej1.setForeground(Color.white);
      ...
      try {
			statement = connection.createStatement();
               String query = "SELECT * FROM sifarnikrobe WHERE sifm=" + 
				   Integer.parseInt(t[5].getText().trim()) + " AND pre=" + Integer.parseInt(pPre);

			try {
		        ResultSet rs = statement.executeQuery( query );
		        rs.next();
				mkdisplej.setText(rs.getString("nazivm"));
		      }
		      catch ( SQLException sqlex ) {
		         	System.out.println("Nema te sifre u sifarniku robe");
		      }
		}     
      catch ( SQLException sqlex ) {
		System.out.println("Greska u trazenju sifre");
      }
 		//.....................................................................................
		finally{
			if (statement != null){
...
//------------------------------------------------------------------------------------------------------------------
    public void UnesiPressed() {
		int odrj,dorj,odmag,domag,konto,sifra;
		String nazkonta,nazrobe;
		String oddat,dodat;
		...
		sifra = Integer.parseInt(t[5].getText().trim());
		...
		int koji = list.getSelectedIndex();
			...
			JInternalFrame pred = new mKartRobe(oddat,dodat,odrj,dorj,
												odmag,domag,
												konto,sifra,
												koji,nazkonta,
												nazrobe);
...
//------------------------------------------------------------------------------------------------------------------
	public void napraviTMPTabelu() {
		...
		strsql = "INSERT INTO tmpmsta(jur,mag,sifm,konto) SELECT DISTINCT " +
           "jur,mag,sifm,konto FROM "+tblpromet+" WHERE "+tblpromet+".pre=" + Integer.parseInt(pPre) +
			" AND "+tblpromet+".konto<>" + kontomal + " AND "+tblpromet+".rbr>0"; 
		izvrsi(strsql);
		strsql = "Update tmpmsta,sifarnikrobe " +
				"SET tmpmsta.nazivm = sifarnikrobe.nazivm "+
				" where tmpmsta.sifm = sifarnikrobe.sifm ";
		izvrsi(strsql);
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildTable() {
		...
		sql = "SELECT jur,mag,sifm,konto,nazivm FROM tmpmsta ORDER BY sifm";
         ...
         statement = connection.createStatement();
         	if ( !t[4].getText().equals( "" )) {
				queryy = "SELECT * FROM sifarnikrobe WHERE sifm=" +
		            Integer.parseInt(t[5].getText().trim());
					ResultSet rs = statement.executeQuery( queryy );
					if(rs.next()){
		         		mkdisplej1.setText(String.valueOf(rs.getString("nazivm")));
						provera = true;
					}else{
						JOptionPane.showMessageDialog(this, "Ne postoji artikal morate ga otvoriti");
					}
			} else {
				JOptionPane.showMessageDialog(this, "Sifra artikla nije uneta");
            }
      }
      catch ( SQLException sqlex ) {
		JOptionPane.showMessageDialog(this, "Greska u proveriRobu:" + sqlex);
      }
		//.....................................................................................
		finally{
			if (statement != null){
...
//===========================================================================
 class mQTMKart extends AbstractTableModel {
	...
	String[] colheads = {"RJ","Mag","\u0160ifra","Konto","Naziv"};
...
class mCLKart implements LayoutManager {
