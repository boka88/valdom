...
public class mKarticeGorivo extends JInternalFrame implements InternalFrameListener
			{
	...
	private JList list,list1;
   	private int n_fields,kkk,kontomal,velemalo=0,prodavnica=1;
//------------------------------------------------------------------------------------------------------------------
    public mKarticeGorivo() {
		super("", true, true, true, true);
		setTitle("Kartice goriva  ");
        ...
        lblUkupno = new JLabel("Ukupno gorivo:");
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildFilterPanel() {
		...
		String a11 = new String("Veleprodaja");
		String a21 = new String("Maloprodaja");
		podaciList1.addElement( a11 );
		podaciList1.addElement( a21 );
...
//------------------------------------------------------------------------------------------------------------------
 public void UnesiPressed() {
		...
		velemalo = list1.getSelectedIndex();
		Ukupno.setText("");
		...
		t[2].getText().trim().length() != 0 )
	{
		...
		String sql;
		sql = "SELECT rbr,datum,ROUND(kolicina,2),stanjekm,predjenokm,ROUND(potrosnja,2)" +
			" FROM gorivo Where brsasije='" + brsasije + "'" +
			" AND datum>='" + konvertujDatum(oddat) + 
			"' AND datum<='" + konvertujDatum(dodat) + "' ORDER BY datum";

		popuniTabelu(sql);
		popuniSaldo(brsasije,konvertujDatum(oddat),konvertujDatum(dodat));
	}else{
...
//------------------------------------------------------------------------------------------------------------------
  public void Stampa() {
		String oddat,dodat,naziv,brsasije="";
	if (t[0].getText().trim().length() != 0 && t[1].getText().trim().length() != 0 && 
		t[2].getText().trim().length()!= 0  )
	{
		brsasije = t[0].getText().trim();
		...
		jPrintKartGorivo mm = new jPrintKartGorivo(connection,brsasije,oddat,dodat);
	}else{
		JOptionPane.showMessageDialog(this, "Prvo popunite sva polja");
		t[0].requestFocus();
	}
...
//------------------------------------------------------------------------------------------------------------------
    public void popuniSaldo(String _brsasije,String _oddat,String _dodat) {
		Statement statement = null;
	  try {
         statement = connection.createStatement();
                String query = "SELECT ROUND(SUM(kolicina),2)" +
					" FROM gorivo WHERE brsasije='" + _brsasije + "'" +
					" AND datum >='" + _oddat + "'" +
					" AND datum <='" + _dodat + "'" +
					" GROUP BY brsasije";
		        ...
		        if(rs.next()){
					Ukupno.setText(rs.getString("ROUND(SUM(kolicina),2)"));
				}else{
					Ukupno.setText("");
				}
      }
         ...
         statement = connection.createStatement();
         	if ( !t[0].getText().equals( "" )) {
				...
				try {
					...
					JOptionPane.showMessageDialog(this, "Ne postoji vozilo morate ga otvoriti:"+sqlex);
					t[0].setText("");
					t[0].requestFocus();
				}
			}     
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildTable() {
		...
		ptbl.setBounds(310,5,380,300);
	   	qtbl = new QTMKartUplata(connection);
		String sql;
		sql = "SELECT rbr,datum,ROUND(kolicina,2),stanjekm,predjenokm,ROUND(potrosnja,2)" +
			" FROM gorivo WHERE brsasije='9999' ORDER BY rbr DESC";
...
//===========================================================================
 class QTMKartUplata extends AbstractTableModel {
	Connection dbconn;
	String[] colheads = {"Rbr","Datum","Koli\u010dina","Stanje km","Pre\u0111eno km","Potro\u0161nja"};
   ...
   public void query(String _sql) {
		...
		try {
            ...
            while ( rs.next() ) {
               ...
               record[2] = rs.getBigDecimal("ROUND(kolicina,2)");
               record[3] = rs.getString("stanjekm");
               record[4] = rs.getString("predjenokm");
               record[5] = rs.getBigDecimal("ROUND(potrosnja,2)");
...
class CLKartUplata implements LayoutManager {
