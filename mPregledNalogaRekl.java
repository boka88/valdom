...
public class mPregledNalogaRekl extends JInternalFrame implements InternalFrameListener
			{
	private QTMPreglNalogaRekl qtbl;
   	private JTable jtbl;
...
//------------------------------------------------------------------------------------------------------------------
    public mPregledNalogaRekl() {
		super("", true, true, true, true);
		setTitle("Pregled radnih naloga - reklamacije");
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildFilterPanel() {
		JPanel p = new JPanel();
		p.setLayout( new CLPreglNalogaRekl() );
...
//------------------------------------------------------------------------------------------------------------------
 public void UnesiPressed() {
	...
	if (t[1].getText().trim().length() != 0 && t[2].getText().trim().length() != 0 )
	{
		...
		izvrsi(strsql);
		strsql = "INSERT INTO tmprnal(rbr,brsasije,datum,cenarada,cenadelova,sifra) " +
				"SELECT rbr,brsasije,datum,cenarada,cenadelova,sifra FROM rnal" +
		...
		strsql = "UPDATE tmprnal,radnici SET tmprnal.koeficient=radnici.koeficient," +
			"tmprnal.ime=radnici.imeprezime WHERE tmprnal.sifra=radnici.sifra";
		izvrsi(strsql);
		strsql = "UPDATE tmprnal SET zarada=cenarada*(100-koeficient)/100";
		izvrsi(strsql);

		
		sql = "SELECT rbr,brsasije,ime,datum,ROUND(cenarada,2),ROUND(cenadelova,2)," +
...
//------------------------------------------------------------------------------------------------------------------
  public void Stampa() {
		...
		t[2].getText().trim().length()!= 0  )
	{
		...
		mPrintListaRekl mm = new mPrintListaRekl(connection,oddat,dodat);
	}else{
...
//------------------------------------------------------------------------------------------------------------------
    public void popuniSaldo(String _oddat,String _dodat) {
	  ...
	  try {
         statement = connection.createStatement();
                String query = "SELECT ROUND(SUM(cenarada),2),ROUND(SUM(cenarada*(100 - koeficient)/100),2)" +
					" FROM tmprnal";
				...
				UkupnoNap.setText(myFormatter.format(rs.getDouble("ROUND(SUM(cenarada*(100 - koeficient)/100),2)")));
      }
      catch ( SQLException sqlex ) {
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildTable() {
		...
		ptbl.setBounds(260,5,450,320);
	   	qtbl = new QTMPreglNalogaRekl(connection);
		String sql;
		sql = "SELECT rbr,brsasije,ime,datum,ROUND(cenarada,2),ROUND(cenadelova,2),ROUND(zarada,2)" +
			" FROM tmprnal" +
...
//===========================================================================
 class QTMPreglNalogaRekl extends AbstractTableModel {
	Connection dbconn;
	String[] colheads = {"Br.naloga","Br.\u0161asije","Radnik","Datum","Cena rada","Cena delova","Zarada"};

//------------------------------------------------------------------------------------------------------------------
   public QTMPreglNalogaRekl(Connection dbc){
...
//------------------------------------------------------------------------------------------------------------------
   public void query(String _sql) {
		...
		try {
            ...
            while ( rs.next() ) {
               ...
               record[6] = rs.getBigDecimal("ROUND(zarada,2)");
 ...
 }//end of class QTMPreglNalogaRekl

}// end of class Konto ====================================================================
class CLPreglNalogaRekl implements LayoutManager {
...
}// end of class CLPreglNalogaRekl
/*
		strsql = "CREATE TEMPORARY TABLE  tmprnal (" +                           
           "rbr  int(11) NOT NULL," +                        
           "brsasije  varchar(50) NOT NULL default ' '," +   
