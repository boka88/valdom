...
public class izvPravna extends JInternalFrame implements InternalFrameListener
			{
...
//------------------------------------------------------------------------------------------------------------------
    public izvPravna() {
        ...
        setTitle("Dospe\u0107e registracije vozila");
...
//------------------------------------------------------------------------------------------------------------------
	...
	protected void fireInternalFrameEvent(int id){
		if (id == InternalFrameEvent.INTERNAL_FRAME_CLOSING ) {
			String strsql = "drop table if exists tmpreg";
			izvrsi(strsql);
...
//------------------------------------------------------------------------------------------------------------------
	public void napraviPomocnuTabelu() {
		...
		strsql = "CREATE TEMPORARY TABLE  tmpreg  (" +               
		...
		izvrsi(strsql);
		strsql = "INSERT INTO tmpreg (brsasije,regbroj,registracija,marka,tip)" +    
			" SELECT brsasije,regbroj,registracija,marka,tip FROM vozila";
		izvrsi(strsql);
		
		//listajPodatke();
		strsql = "UPDATE tmpreg SET novidatum=date_add(registracija,interval 1 year)";
		izvrsi(strsql);

		strsql = "UPDATE tmpreg SET brojdana=(TO_DAYS(novidatum) - TO_DAYS(now()))";
		izvrsi(strsql);
  }
//------------------------------------------------------------------------------------------------------------------
  public void listajPodatke() {
	String sssql="",ssql="SELECT * FROM tmpreg",novidat="",novidat1="",brsasije="";
		Statement statement = null;
      try {
				statement = connection.createStatement();
				ResultSet rs = statement.executeQuery( ssql );
				while ( rs.next() ) {
					 brsasije = rs.getString("brsasije").trim();
					 novidat = konvertujDatumIzPodataka(rs.getString("registracija"));
					 novidat1 =  konvertujDatum(NoviDatum(novidat,365));
					 
					 sssql = "UPDATE tmpreg SET novidatum='" + novidat1 + "'" +
		...
		JOptionPane.showMessageDialog(this, "Greska u listanju podataka :"+sqlex);
      }
		//.....................................................................................
		finally{
			if (statement != null){
...
//------------------------------------------------------------------------------------------------------------------
    public void Stampa() {
		jPrintReg kkl = new jPrintReg(connection);
	}
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
    public void buildTable() {
		...
		String sql;
		sql = "SELECT * FROM tmpreg ORDER BY brojdana";
	   	qtbl.query(sql);
...
//===========================================================================
 class izvQTM3 extends AbstractTableModel {
	...
	String[] colheads = {"Reg.broj","Broj \u0161asije","Datum reg.","Novi datum","Preostalo dana"};
//------------------------------------------------------------------------------------------------------------------
   ...
   public void query(String _sql) {
        ...
        Statement statement = dbconn.createStatement();
            ...
            while ( rs.next() ) {
               ...
               record[2] = konvertujDatumIzPodataka(rs.getString("registracija"));
               record[3] = konvertujDatumIzPodataka(rs.getString("novidatum"));
               record[4] = rs.getString("brojdana");
