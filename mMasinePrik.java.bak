...
public class mMasinePrik extends JDialog{
   ...
   public mMasinePrik(Connection _connection,int kojaf)	{
   ...
   public void Kreni() {
		...
		switch(kojaForma){
			case 1:
				...
				mUnosPrijem.dispMT.setText(kojiNazKonta);
				break;
			case 2:
...
//--------------------------------------------------------------------------------------
  public void proveriPoNazivu(){
			if ( !naziv.getText().equals( "" )) {
	            String sql = "SELECT * FROM masine WHERE naziv" +
...
//=======================================================================================
 class mQTMTrosPrik extends AbstractTableModel {
   Connection dbconn;
	String[] colheads = {"\u0160ifra","Naziv ma\u0161ine","Karton"};
