...
public class izvLekarski extends JInternalFrame implements InternalFrameListener
			{
...
//------------------------------------------------------------------------------------------------------------------
    public izvLekarski() {
        ...
        setTitle("Dospe\u0107e lekarskog pregleda");
...
//------------------------------------------------------------------------------------------------------------------
	...
	protected void fireInternalFrameEvent(int id){
		if (id == InternalFrameEvent.INTERNAL_FRAME_CLOSING ) {
			String strsql = "drop table if exists tmplek";
			izvrsi(strsql);
...
//------------------------------------------------------------------------------------------------------------------
	public void napraviPomocnuTabelu() {
		...
		strsql = "CREATE TEMPORARY TABLE  tmplek  (" +               
           ...
           "lekarskimes varchar(50) default ' '," +                    
           "brojdana int(7) default '0'," +   
           "UNIQUE KEY  jmbg  ( jmbg )" +   
           ") ENGINE=MyISAM DEFAULT CHARSET=utf8";
		izvrsi(strsql);
		strsql = "INSERT INTO tmplek (jmbg,imeprezime,datumzadlek,lekarskimes)" +    
			" SELECT jmbg,imeprezime,datumzadlek,lekarskimes FROM radnici" +
			" WHERE lekarskimes>0";
		izvrsi(strsql);
		
		//listajPodatke();
		strsql = "UPDATE tmplek SET novidatum=date_add(datumzadlek,interval lekarskimes month)";
		izvrsi(strsql);

		strsql = "UPDATE tmplek SET brojdana=(TO_DAYS(novidatum) - TO_DAYS(now()))";
		izvrsi(strsql);
  }
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
    public void Stampa() {
		jPrintLek kkl = new jPrintLek(connection);
	}
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
    public void buildTable() {
		...
		String sql;
		sql = "SELECT * FROM tmplek ORDER BY brojdana";
	   	qtbl.query(sql);
...
//===========================================================================
 class izvQTM3 extends AbstractTableModel {
	...
	String[] colheads = {"JMBG","Ime i prezime","Datum lek.","Novi datum","Preostalo dana"};
//------------------------------------------------------------------------------------------------------------------
   ...
   public void query(String _sql) {
        ...
        Statement statement = dbconn.createStatement();
            ...
            while ( rs.next() ) {
               ...
               record[2] = konvertujDatumIzPodataka(rs.getString("datumzadlek"));
