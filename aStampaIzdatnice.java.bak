...
public class aStampaIzdatnice extends JInternalFrame implements InternalFrameListener{
...
//------------------------------------------------------------------------------------------------------------------
	public void Obrada() {
		...
		sql = "drop table if exists tmpizdatnice";
		izvrsi(sql);
		sql = "CREATE TEMPORARY TABLE tmpizdatnice SELECT DISTINCT mag,brun,datum FROM mprom1 " +
			" WHERE vrdok=3 AND mag<99";
		izvrsi(sql);
  }
//------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------
   public void Kreni() {
	...
	if (t[0].getText().trim().length()>0 && t[1].getText().trim().length()>0 && 
		...
		jPrintPrijemnica pp = new jPrintPrijemnica(connection,jurm,magm,brdokm,vrdokm,3);
...
//------------------------------------------------------------------------------------------------------------------
    public void buildTable() {
		...
		sql = "SELECT * FROM tmpizdatnice ORDER BY brun";
...
//===========================================================================
 class QTMPopis extends AbstractTableModel {
	Connection dbconn;
	String[] colheads = {"Magacin","Broj izdatnice","Datum"};
...
class mCLLIzdatniceStampa implements LayoutManager {
...
}// end of class mCLLIzdatniceStampa
