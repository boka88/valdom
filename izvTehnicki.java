...
public class izvTehnicki extends JInternalFrame implements InternalFrameListener
			{
...
//------------------------------------------------------------------------------------------------------------------
    public izvTehnicki() {
        ...
        setTitle("Dospe\u0107e tehni\u010dkih pregleda vozila");
...
//------------------------------------------------------------------------------------------------------------------
	...
	protected void fireInternalFrameEvent(int id){
		if (id == InternalFrameEvent.INTERNAL_FRAME_CLOSING ) {
			//String strsql = "drop table if exists tmpteh";
			//izvrsi(strsql);
...
//------------------------------------------------------------------------------------------------------------------
	public void napraviPomocnuTabelu() {
		...
		strsql = "CREATE TEMPORARY TABLE  tmpteh  (" +               
		...
		izvrsi(strsql);
		strsql = "INSERT INTO tmpteh (brsasije,regbroj,tehpregled,marka,tip,brojmesteh)" +    
			" SELECT brsasije,regbroj,tehpregled,marka,tip,brojmesteh FROM vozila WHERE "+
			" brojmesteh>0";
		izvrsi(strsql);
		
		//listajPodatke();
		strsql = "UPDATE tmpteh SET novidatum=date_add(tehpregled,interval brojmesteh month)";
		izvrsi(strsql);

		strsql = "UPDATE tmpteh SET brojdana=(TO_DAYS(novidatum) - TO_DAYS(now()))";
		izvrsi(strsql);
  }
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
    public void Stampa() {
		jPrintTeh kkl = new jPrintTeh(connection);
	}
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
    public void buildTable() {
		...
		String sql;
		sql = "SELECT * FROM tmpteh ORDER BY brojdana";
	   	qtbl.query(sql);
   ...
   public void query(String _sql) {
        ...
        Statement statement = dbconn.createStatement();
            ...
            while ( rs.next() ) {
               ...
               record[2] = konvertujDatumIzPodataka(rs.getString("tehpregled"));
