...
public class mFinanKupciRn extends JInternalFrame implements InternalFrameListener
			{
	private QTMFinanKupciRn qtbl;
   	private JTable jtbl;
...
//------------------------------------------------------------------------------------------------------------------
    public mFinanKupciRn() {
		super("", true, true, true, true);
		setTitle("Kartica kupca - radni nalozi");
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildFilterPanel() {
		JPanel p = new JPanel();
		p.setLayout( new CLFinanKupciRn() );
 ...
 public void UnesiPressed() {
		...
		t[3].getText().trim().length() != 0 )
	{
		...
		izvrsi(strsql);
			...
			strsql = "INSERT INTO tmpduznici(datum,brrac,dug,opisradova) " +
				"SELECT datum,rbr,cenarada+cenadelova,opisradova FROM rnal" +
		...
		sql = "SELECT datum,brrac,ROUND(dug,2),opisradova" +
...
//------------------------------------------------------------------------------------------------------------------
  public void Stampa() {
	...
	if (t[0].getText().trim().length() != 0 )
	{
		...
		mPrintKartRn mm = new mPrintKartRn(connection,oddat,dodat,kupac,naziv);
	}else{
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildTable() {
		...
		ptbl.setBounds(300,5,480,320);
	   	qtbl = new QTMFinanKupciRn(connection);
		String sql;

		sql = "SELECT datum,brrac,ROUND(dug,2),opisradova FROM tmpduznici" +
			" ORDER BY datum";
...
//===========================================================================
 class QTMFinanKupciRn extends AbstractTableModel {
	Connection dbconn;
	String[] colheads = {"Br.nal.","Datum","Opis radova","Iznos"};

//------------------------------------------------------------------------------------------------------------------
   public QTMFinanKupciRn(Connection dbc){
...
//------------------------------------------------------------------------------------------------------------------
   public void query(String _sql) {
		...
		try {
            ...
            while ( rs.next() ) {
               ...
               record[2] = rs.getString("opisradova");
 ...
 }//end of class QTMFinanKupciRn

}// end of class Konto ====================================================================
class CLFinanKupciRn implements LayoutManager {
