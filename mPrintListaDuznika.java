...
public class mPrintListaDuznika {
  ...
  public mPrintListaDuznika(Connection _connection,
						String _oddat,
						String _dodat) {
...
private void writeHeader() 
{
	...
	// pocinju stavke  -------------------------------------------------------------------------
		output.write("      <fo:region-body margin-top=\"40mm\"\n"); 
	...
	//-------------------- prva stavka je visina u koju mora stati zaglavlje   -------------
		output.write("      <fo:region-before extent=\"40mm\"/>\n"); 
...
//------------------------------------------------------------------------------------------
 private void writeDetails() {
        ...
        try {
			...
			output.write("              <fo:block>" + konvUnicode("Lista du\u017enika") + "\n"); 
			output.write("              </fo:block>\n"); 
			output.write("            </fo:table-cell>\n"); 
			// strana -----------------------------------------------------------------------
			output.write("          </fo:table-row>\n"); 
...
//---------------------------------------------------------------------------------
 private void popuniPodatkeIzBaze() {
	String sql = "SELECT brsasije,nazivk,format(duguje,2),format(potrazuje,2),format(saldo,2) " +
		" FROM tmpduznici ORDER BY nazivk";
     ...
     try {
        ...
        while ( rs.next() ) {
               ...
               String[] record = new String[5];
				...
				record[2] = rs.getString("format(duguje,2)");
				record[3] = rs.getString("format(potrazuje,2)");
				record[4] = rs.getString("format(saldo,2)");
...
//---------------------------------------------------------------------------------
 private void popuniUkupno() {
		String sql = "SELECT ROUND(SUM(saldo),2) FROM tmpduznici ";
      ...
      try {
        ...
        if ( rs.next() ) {
               ...
               String[] record = new String[7];
				record[0] = rs.getString("ROUND(SUM(saldo),2)");
