...
public class regListFizicka extends JInternalFrame implements InternalFrameListener
			{
	private reglistQTM2 qtbl;
   	private JTable jtbl;
    ...
    public static JFormattedTextField t[],mmoj,displej,imakuku,razlog;
   	...
   	int n_fields;
//------------------------------------------------------------------------------------------------------------------
    public regListFizicka() {
		super("", true, true, true, true);
        setTitle("\u0160tampa registracionog lista Fizi\u010dka lica");
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildNaslov() {
		...
		//naslov.setBounds(0,0,740,30);

		JLabel disp = new JLabel("Ime i prezime:");
		//disp.setBounds(5,5,70,20);

		displej = new JFormattedTextField(); 
		displej.setColumns(30);
		//displej.setBounds(10,5,150,20);
...
//------------------------------------------------------------------------------------------------------------------
    public void Stampa() {
		...
		if ( t[0].getText().trim().length()!=0 && 
			...
			PrintRegFizicka kkl = new PrintRegFizicka(jmbg,regoznaka,razl);}
...
//------------------------------------------------------------------------------------------------------------------
    public void buildTable() {
		...
		ptbl.setBorder( new TitledBorder("Podaci") );
	   	...
	   	qtbl = new reglistQTM2(connection);
		String sql;
		sql = "SELECT * FROM vozila WHERE LENGTH(TRIM(JMBG))>10";
	   	qtbl.query(sql);
