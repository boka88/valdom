...
public class mKartRobeV extends JInternalFrame implements InternalFrameListener
			{
	private Object[][] data;
	...
	private int odjur,dojur,odmag,domag,konto,koji;
	private String pPre,nazivPre,nazvozila,nazrobe,oddat,dodat;
	...
	private JList list;
	private String tblpromet="mprom",sifra="";

   	int n_fields;
//------------------------------------------------------------------------------------------------------------------
    public mKartRobeV(	String _oddat,
						String _dodat,
						int _odjur,
						int _dojur,
						int _odmag,
...
//==================================================================================================================
 public void Obrada() {
	...
	strsql = "CREATE TEMPORARY TABLE tmpkarticarobe (" +
	  ...
	  " nazivm  varchar(30) default ' '," +
	  " brrac  double default 0," +
	  " datum  datetime default '0000-00-00'," +
	  " ui  tinyint(3) unsigned default 0," +
	  " vrdok  smallint(6) default 0," +
	  " brdok  int(11) default 0," +
	  " kolic  double default 0," +
	  " cena  double default '0'," +
	  " cenad  double default '0'," +
	  " vredn  double default '0'," +
	  " rbr  int(11) default 0," +
	  " brun  double default 0," +
	  " kupacbr  int(11) default 0," +
	  " fak  tinyint(3) unsigned default '0'," +
	  ...
	  " kolicme  double default '0'" +
		...
		strsql = "INSERT INTO tmpkarticarobe(vozilo,konto,sifm,oznaka,jur,mag,ui,vrdok,brdok,datum,kolic," +
		"cena,vredn,rbr,brun,kupacbr,brrac,fak,sar,tarbr,por1,por2,por3,zav," +
		"mar,taksa,preneto,pcena,pcena1,pvredn,rabat,ulazi,mtros) " +
		"SELECT vozilo,konto,sifm,oznaka,jur,mag,ui,vrdok,brdok,datum,kolic,cena,vredn," +
		"rbr,brun,kupacbr,brrac,fak,sar,tarbr,por1,por2,por3," +
		"zav,mar,taksa,preneto,pcena,pcena1,pvredn,rabat,ulazi,mtros  " +
		"FROM "+tblpromet+" WHERE "+tblpromet+".pre = 1" + 
		...
		" and "+tblpromet+".vozilo = '" + sifra + "'";
		izvrsi(strsql);


 		strsql = "UPDATE tmpkarticarobe SET tmpkarticarobe.kolicme = tmpkarticarobe.kolic " +
		"WHERE tmpkarticarobe.ui = 1 and cena=0";
		izvrsi(strsql);
 		strsql = "UPDATE tmpkarticarobe SET tmpkarticarobe.kolicul = tmpkarticarobe.kolic, " +
	    "tmpkarticarobe.vredul = tmpkarticarobe.vredn " +
		"WHERE (tmpkarticarobe.ui = 0 or tmpkarticarobe.ui = 1)  ";
		izvrsi(strsql);
 		strsql = "UPDATE tmpkarticarobe SET tmpkarticarobe.koliciz = tmpkarticarobe.kolic, " +
	    "tmpkarticarobe.vrediz = tmpkarticarobe.vredn " +
		"WHERE (tmpkarticarobe.ui = 2 or tmpkarticarobe.ui = 3)  ";
		izvrsi(strsql);


		strsql = "UPDATE tmpkarticarobe SET tmpkarticarobe.kolic = tmpkarticarobe.kolic * -1," +
		...
		"AND tmpkarticarobe.vrdok = dokumentimaterijalno.vrdok";
			izvrsi(strsql);
			...
			try {
				...
				int i=0;
					...
					while ( rs.next() ) {
					  ...
					  saldox  =  saldox + rs.getDouble("kolic");
...
//------------------------------------------------------------------------------------------------------------------
    public void Stampa() {
		...
		jPrintKartV jjj = new jPrintKartV(connection,odjur,dojur,
												odmag,domag,
												sifra,
												koji,nazvozila);
	}
...
//------------------------------------------------------------------------------------------------------------------
    public void popuniSaldo() {
	  ...
	  try {
		...
		JOptionPane.showMessageDialog(this, "Greska u SUM1");
      }
  }
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildTable() {
		...
		sql = "SELECT oznaka,opis,jur,mag,vrdok,brdok,datum,ROUND(kolic,2)," +
			"ROUND(cena,2),ROUND(vredn,2),brun,rbr,kupacbr,brrac FROM tmpkarticarobe order by datum , ui";
		}
		else{

			sql = "SELECT oznaka,opis,jur,mag,vrdok,brdok,datum,ROUND(kolic,2)," +
			"ROUND(cena,2),ROUND(vredn,2),brun,rbr,kupacbr,brrac FROM tmpkarticarobe order by brdok , ui, datum";}
	   try{
	   		qtbl.query(sql);
...
//===========================================================================
 class QTMKartProm extends AbstractTableModel {
	...
	String[] colheads = {"Ozn","Opis","RJ","Mag.","Vrs.dok.",
						"Br.dok.","Datum","Kolicina","Cena",
						"Vrednost","Br.unosa","Br.stavke","Saradnik","Ra\u010dun"};
        ...
        statement = dbconn.createStatement();
            ...
            while ( rs.next() ) {
               ...
               Object[] record = new Object[14];
				...
				record[7] = rs.getBigDecimal("ROUND(kolic,2)");
...
class CLKartRobeV implements LayoutManager {
