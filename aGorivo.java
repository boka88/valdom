...
public class aGorivo extends JInternalFrame implements InternalFrameListener
			{
...
//uzima sledeci broj stavke 
   public void novaStavka(){ 
      ...
      try {
         statement = connection.createStatement();
               		String query = "SELECT MAX(rbr) FROM gorivo";
			try {
	         ResultSet rs = statement.executeQuery( query );
				if(rs.next()){
	...
	public void AddRecord() {
      ...
      try {
		 ...
		 if ( t[0].getText().trim().length() != 0 && 
            ...
            String query = "INSERT INTO gorivo (rbr,brsasije,regbroj,datum,kolicina,stanjekm," +
				"predjenokm,potrosnja) VALUES(" +
				...
				Double.parseDouble(t[6].getText().trim()) + ")";			
			...
			//JOptionPane.showMessageDialog(this, "Slog je unet");
				...
				String upit = "SELECT rbr,brsasije,regbroj,datum,ROUND(kolicina,2),stanjekm,predjenokm," +
					"ROUND(potrosnja,2) FROM gorivo" +
					" ORDER BY rbr DESC";
				popuniTabelu(upit);
				NoviPressed();
			}     
      ...
      try {
		 ...
		 if ( !t[0].getText().equals( "" ) && !t[1].getText().equals( "" )) {
               query = "UPDATE gorivo SET " +
				...
				"predjenokm=" + Double.parseDouble(t[5].getText().trim()) + "," +
				"potrosnja=" + Double.parseDouble(t[6].getText().trim()) + 
...
//------------------------------------------------------------------------------------------------------------------
	public void DeleteRecord() {
      ...
      try {
         statement = connection.createStatement();
         if ( !t[0].getText().equals( "" ) && !t[1].getText().equals( "" ) ) {
               	String query = "DELETE FROM gorivo WHERE  rbr=" + 
						Integer.parseInt(t[0].getText().trim());
	...
	public void FindRecord() {
      ...
      try {
		 ...
		 if ( !t[0].getText().equals( "" )) {
			String query = "SELECT * FROM gorivo WHERE rbr=" + koji;
			...
			try {
		         ...
		         if(rs.next()){
					...
					t[5].setText(rs.getString("predjenokm"));
					t[6].setText(rs.getString("potrosnja"));
...
//------------------------------------------------------------------------------------------------------------------
    public JPanel buildTable() {
		...
		String sql;
		sql = "SELECT rbr,brsasije,regbroj,datum,ROUND(kolicina,2),stanjekm,predjenokm," +
					"ROUND(potrosnja,2) FROM gorivo" +
					" ORDER BY rbr DESC";
	   	qtbl.query(sql);
	...
	public void Akcija(JFormattedTextField e) {
		...
		source = e;
				...
				if (source == t[0]){
					...
					try{
						potrosnja = (Double.parseDouble(t[3].getText().trim())
							/Double.parseDouble(t[5].getText().trim())*100);
					}catch(Exception ee){
						JOptionPane.showMessageDialog(this, "Podaci nisu u redu:"+ee);
						t[5].setSelectionStart(0);
						t[5].requestFocus();
					}
						t[6].setText(myFormatter.format(potrosnja));
...
//===========================================================================
 class mQTMUplate extends AbstractTableModel {
	Connection dbconn;
	String[] colheads = {"Rbr","Br.\u0161asije","Reg.br.","Datum","Koli\u010dina","Stanje km","Predjeno km","Potro\u0161nja"};
   ...
   public void query(String _sql) {
        ...
        statement = dbconn.createStatement();
            ...
            while ( rs.next() ) {
               ...
               record[6] = rs.getBigDecimal("predjenokm");
               record[7] = rs.getBigDecimal("ROUND(potrosnja,2)");
...
class MCLGorivo implements LayoutManager {
