...
public class aNadji {
 ...
 //-----------------------------------------------------------------------------------------------------
    public int mNadjiBrojUnosa(Connection connection,int _pre,int _jur,int _mag,
								String _vrdok,int _ui,int _indikator) {
		int brunosa=1;
		Statement statement=null;
      try {
			statement = connection.createStatement();
               String query = "SELECT MAX(brun) FROM mprom" + String.valueOf(_pre) + 
				   " WHERE" +
				   " pre=" + _pre + " AND " +
				   " jur=" + _jur + " AND " +
				   " mag=" + _mag + " AND " +
 ...
 //-----------------------------------------------------------------------------------------------------
    public int mNadjiNovuStavku(Connection connection,int _pre,int _jur,int _mag,
								int _brun,int _ui,int _indikator,String _vrdok) {
		int stavka=1;
		Statement statement=null;
      try {
			statement = connection.createStatement();
               String query = "SELECT MAX(rbr) FROM mprom" + String.valueOf(_pre) + 
				   " WHERE" +
				   " pre=" + _pre + " AND " +
				   " jur=" + _jur + " AND " +
				   " mag=" + _mag + " AND " +
    ...
    public double mNadjiPRC(Connection connection,int _pre,int _jur,int _mag,
      ...
      try {
			statement = connection.createStatement();
               ...
               String query = "SELECT ROUND((SUM(kolicul*cena)-SUM(koliciz*cena)+SUM(vrednnivel))" +
				   "/(SUM(kolicul)-SUM(koliciz)-SUM(kolicme)),2)" +
				   " FROM mprom" + String.valueOf(_pre) + 
				   " WHERE pre=" + _pre + 
				   " AND jur=" + _jur + " AND " +
				   " mag=" + _mag + " AND " +
				   " sifm=" + _sifm + " AND " +
      ...
      try {
			statement = connection.createStatement();
              //String query = "SELECT ROUND(SUM(kolicul)-SUM(koliciz),2) FROM mprom WHERE pre=" + _pre + 
               String query = "SELECT ROUND(SUM(kolicul)-SUM(koliciz)-SUM(kolicme),2) FROM mprom" + String.valueOf(_pre) + 
				   " WHERE pre=" + _pre + 
				   " AND jur=" + _jur + " AND " +
				   " mag=" + _mag + " AND " +
				   " sifm=" + _sifm + " AND " +
...
//------------------------------------------------------------------------------------------------------------------
	public void obrisiPomocniNultiSlog(Connection connection,int _pre,int _jur,int _mag,
      ...
      try {
         statement = connection.createStatement();
               	String query = "DELETE FROM mprom" + String.valueOf(_pre) + 
				   " WHERE pre=" + _pre + " AND " +
					" jur=" + _jur+ " AND " +	
					" mag=" + _mag + " AND " +	
					" ui=" + _ui + " AND " +	
...
//------------------------------------------------------------------------------------------------------------------
	public void unesiPomocniNultiSlog(Connection connection,int _pre,int _jur,int _mag,
      ...
      try {
         statement = connection.createStatement();
               String query = "INSERT INTO mprom" + String.valueOf(_pre) + 
				" (pre,jur,mag,ui,indikator,brun,sifm,konto,rbr,datum,z,vrdok)" +
				" VALUES (" + _pre + "," +		
				_jur + "," +		
			...
			JOptionPane.showMessageDialog(null, "Greska u unosu pomocnog sloga:"+sqlex);
      }
 		//.....................................................................................
		finally{
			if (statement != null){
...
//--------------------------------------------------------------------------------------
    public int nadjiBrojPreduzeca(Connection connection) {
		int brpred=1;
		Statement statement=null;
      try {
			statement = connection.createStatement();
               String query = "SELECT MAX(Pre) FROM preduzeca"; 
			try {
		        ResultSet rs = statement.executeQuery( query );
		        if(rs.next()){
					brpred = rs.getInt(1);
