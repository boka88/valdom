...
public class jPrintKartGorivo {
	...
	private String brsasije="",strvaluta="",strdatump="",oddat,dodat;
	...
	public jPrintKartGorivo(Connection _connection, String _brsasije,
		String _oddat,String _dodat){
		...
		try
		{
			JasperDesign jasperDesign = JasperManager.loadXmlDesign("print/kartgorivo.jrxml");
			...
			parameters.put("vrstavozila", vrstavozila);
			parameters.put("brojsasije", brsasije);
...
//--------------------------------------------------------------------------------------
	private JasperPrint uzmiPodatke(JasperReport _jasperReport,Map _parameters){
		...
		String sql = "SELECT * FROM gorivo where brsasije='" + brsasije + 
			"' ORDER BY datum";
		try {
      ...
      try {
         statement = connection.createStatement();
			...
			try {
		         ResultSet rs = statement.executeQuery( query );
		         if(rs.next()){
					registracija = rs.getString("regbroj"); 
