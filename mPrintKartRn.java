...
public class mPrintKartRn {
  ...
  public mPrintKartRn(Connection _connection,
						String _oddat,
						String _dodat,
						String _kupac,
						String _naziv) {
...
//------------------------------------------------------------------------------------------
 private void writeDetails() {
        ...
        try {
			...
			output.write("              <fo:block>" + konvUnicode("Kartica kupca - svi nalozi") + "\n"); 
			output.write("              </fo:block>\n"); 
			output.write("            </fo:table-cell>\n"); 
			// strana -----------------------------------------------------------------------
			output.write("          </fo:table-row>\n"); 
...
//---------------------------------------------------------------------------------
 private void popuniPodatkeIzBaze() {
	...
	String sql = "SELECT datum,brrac,opisradova,ROUND(dug,2) " +
     ...
     try {
        ...
        while ( rs.next() ) {
               ...
               record[0] = konvertujDatumIzPodataka(rs.getString("datum"));
               record[1] = rs.getString("brrac");
